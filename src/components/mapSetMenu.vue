...
export default {
  ...
  data() {
    return {
      ...
      menuList: [
        ...
        { id: 5, icon: "ios-briefcase-outline", title: "显示网格", color: "#151c2d"},
      ]
    };
  },
  filters: {
  ...
  methods: {
    ...
    5() {
      if (this.menuList[0][4].title === "显示网格") {
        this.$parent.$refs.mapView.visible.layer = true;
        this.$set(this.menuList[0][4], 'title', "关闭网格");
      } else {
        this.$parent.$refs.mapView.visible.layer = false;
        this.$set(this.menuList[0][4], 'title', "显示网格");
      }
    },
    //格式化菜单数据,以6来均分
