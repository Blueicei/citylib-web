      ...
      >
        <el-tab-pane label="卡口" name="first">
          ...
          <el-table
            ...
            :data="
              camTable.slice((cam_current_page - 1) * 5, cam_current_page * 5)
            "
            ...
            style="width: 100%"
          >
            <el-table-column prop="camId" align="center" label="摄像头">
            </el-table-column>
            <el-table-column prop="count" align="center" sortable label="流量">
            </el-table-column>
            <el-table-column prop="camAddress" align="center" label="地址">
            </el-table-column>
          </el-table>
          <el-pagination
        ...
        <el-tab-pane label="轨迹" name="second">
          ...
          >
            ...
            <el-table-column prop="distance" align="center" label="行程距离/m">
            </el-table-column>
            <el-table-column prop="color" label="颜色">
              <template slot-scope="scope">
                <el-color-picker
                  ...
                  size="mini"
                  show-alpha
                  @change="handleTraRowClick(scope.row)"
                ></el-color-picker>
              </template>
            </el-table-column>
          </el-table>
          ...
          <div v-show="showPoint">
            ...
            <el-table
              ...
              :data="
                pointTable.slice(
                  (point_current_page - 1) * 5,
                  point_current_page * 5
                )
              "
              ...
              style="width: 100%"
            >
              <el-table-column prop="photoTime" align="center" label="时间">
                <template slot-scope="scope">
              ...
              <el-table-column prop="direction" align="center" label="方向">
              </el-table-column>
            </el-table>
            <el-pagination
        ...
        <el-tab-pane label="流向" name="third">
          ...
          <el-table
            ...
            :data="
              clusterTable.slice(
                (cluster_current_page - 1) * 5,
                cluster_current_page * 5
              )
            "
            ...
            style="width: 100%"
          >
            <el-table-column prop="name" align="center" label="聚合点">
            </el-table-column>
            <el-table-column
              sortable
              prop="startCount"
  ...
  methods: {
    ...
    clickHandler(e) {
      ...
      this.camera_info.forEach((item) => {
        if (
          item.lng === this.infoPosition.lng &&
          item.lat === this.infoPosition.lat
        ) {
    ...
    getCamCountByCar() {
      ...
      let max = 0;
      this.axios
        .get("/camTra/getCamCountByCar?carNumber=" + this.carNumber)
        .then((res) => {
          res.data.msg.forEach((item) => {
            if (item.count > max) {
              max = item.count;
            }
            this.heatMapInfo.push({
      ...
      if (!this.firstFlag) {
        ...
        for (const key of Object.keys(clusterFlowLine)) {
          if (key == val && clusterFlowLine[key].length > 0) {
            ...
            potArr.push(
              new BMap.Point(
                clusterFlowLine[key][0].lng,
                clusterFlowLine[key][0].lat
              )
            );
            potArr.push(
              new BMap.Point(
                clusterFlowLine[key][1].lng,
                clusterFlowLine[key][1].lat
              )
            );
    ...
    getTraByCar() {
      ...
      this.traTable = [];
      this.axios
        .get("/camTra/getTraByCar?carNumber=" + this.carNumber)
        .then((res) => {
    ...
    initCluster() {
      ...
      let markerCluster = new BMapLib.MarkerClusterer(map, {
        markers: markers,
    ...
    getClusterFlow() {
      ...
      for (let i = 0; i < traTable.length; i++) {
        ...
        const endPoint =
          traTable[i].points[traTable[i].points.length - 1].camId;
