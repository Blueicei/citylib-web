<template>
  <div class="main-cont" :style="mainContStyle">
    <div class="main-map" id="main-map" @click="hideMenu" :style="mainMapStyle">
      <mapView ref="mapView" :points="points" :point="point" :lines="lines"
               :polylinePath="polylinePath" :POI="POI" :end="end" :pulseLines="pulseLines">
    ...
    <div class="data-monitor">
      <cityInfo v-show="isMain" ></cityInfo>
      <info v-if="isInfo" ref="info" :camList="camList"></info>
      <count v-show="isCount" ref="count" :camList="camList"></count>
      <cars v-if="isCars" ref="cars" :camList="camList"></cars>
      <car v-show="isCar" ref="car" :camList="camList"></car>
      <city v-show="isCity" ref="city" :camList="camList"></city>
      <cityGrid v-show="isGrid" ref="grid"></cityGrid>
    </div>
...
import cityInfo from "@/components/mapLayer/info.vue"
...
import city from "@/components/DataShow/cityCurrency.vue"
import cityGrid from "@/components/DataShow/cityGrid.vue"
  ...
  methods: {
    init(){
      getAllCamInfo().then((response) => {
        // console.log('points',response.msg)
        this.points = response.msg
        for (var i=0; i<this.points.length; i++){
          this.points[i].id = i
          this.points[i].selected = 0
        }
        this.$refs.count.allData  = this.points
        this.$refs.city.allData  = this.points
      })
